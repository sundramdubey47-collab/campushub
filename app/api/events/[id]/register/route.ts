...
export async function POST(
  ...
  try {
    ...
    if (existing) {
      ...
      return NextResponse.json({ registration: existing, qrImage: qrImageDataUrl })
    }
    ...
    return NextResponse.json({ error: "Something went wrong while registering. Please try again." }, { status: 500 })
  }
}
...
export async function GET(
  ...
  try {
    ...
    console.error("Event registration status error:", err)
    return NextResponse.json({ registered: false })
  }
}