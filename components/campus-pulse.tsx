...
export function CampusPulse() {
  const [data, setData] = useState<{ newNotes: number; testAttempts: number; chatMessages: number } | null>(null)

  useEffect(() => {
    fetch("/api/dashboard/pulse").then((r) => r.json()).then(setData)
  }, [])

  if (!data || (data.newNotes === 0 && data.testAttempts === 0 && data.chatMessages === 0)) return null
